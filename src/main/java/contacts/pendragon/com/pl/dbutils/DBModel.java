...
public abstract class DBModel implements Comparable<DBModel> {
    ...
    private List<DBField> getDBFields(List<Field> fields) throws IllegalAccessException {
        ...
        for (Field field : fields) {
            //getting object of DBField for this object; casting is crucial
    ...
    private String getInsertStatemant()
            throws IllegalAccessException, DBModelException {
        SQLDict sqlDict = sqlDictFactory.getSQLDict();
        String baseSqlStatement = sqlDict.insertStatement;
        String sql;
        ...
        sql = String.format(baseSqlStatement, this.model.toUpperCase(),
    ...
    private String getUpdateStatement()
            throws IllegalAccessException, DBModelException {
        SQLDict sqlDict = sqlDictFactory.getSQLDict();
        String baseSqlStatement = sqlDict.updateStatement;
        String sql;
    ...
    private String getSelectSimpleStatement(String[] order_by, String sort_type)
            throws IllegalAccessException, DBModelException {
        SQLDict sqlDict = sqlDictFactory.getSQLDict();
        ...
        if (order_by != null && order_by.length > 0) {
            ...
            baseSqlStatement = sqlDict.selectSimpleStatementOrdered;
            sql = String.format(baseSqlStatement,
                    this.model.toUpperCase(), sqlWhere.toString(), sqlOrder.toString());
        } else {
            // non ordered statement
            baseSqlStatement = sqlDict.selectSimpleStatement;
            sql = String.format(baseSqlStatement,
    ...
    private List<DBModel> runSelectSimple(Connection dbConn, String[] order_by, String sort_type)
        ...
        String sql = this.getSelectSimpleStatement(order_by, sort_type);
        ...
        // here we close statement the connection must be close in method invoking this method
        try (PreparedStatement stmt = dbConn.prepareStatement(sql)) {
            ...
            // setting value for pk - this will by always last position
    ...
    private void runInsert(Connection dbConn)
            throws IllegalAccessException, SQLException, DBModelException {
        String sql = this.getInsertStatemant();
        List<Field> fields = this.getNotNullFields(this.modelFields);
        List<DBField> dbFields = this.getDBFields(fields);
    ...
    public void save()
        ...
        try (Connection conn = dbFactory.getDBConnection()) {
            if (pkField.getValue() == null) {
                ...
                // primary key has value = Update object in db
                this.runUpdate(conn);
            }
        }
    }
